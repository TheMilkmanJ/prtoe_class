...
# - Very correlated parameters (H0-omega_cdm, sigma8-S8 etc.)
# - Low effective sample density in some projections during early monitoring
# We still get good contours via GetDist's fallback + our manual smoothing.
warnings.filterwarnings("ignore", message=r".*2D kernel density bandwidth optimizer failed.*")
getdist_logger = logging.getLogger("getdist")
if getdist_logger:
    getdist_logger.setLevel(logging.ERROR)
plots_logger = logging.getLogger("getdist.plots")
if plots_logger:
    plots_logger.setLevel(logging.ERROR)
...
def main(args, first_run=False):
    ...
    try:
        ...
        # Use fixed smoothing scale for 2D to make contours robust against
        # the degeneracy valley + forced unit weights. Avoids the optimizer
        # failures we see in early chains or highly correlated directions.
