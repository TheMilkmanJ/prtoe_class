...
int perturbations_timesampling_for_sources(
  ...
  if ((ppt->has_cmb == _TRUE_)||(ppt->has_perturbed_recombination == _TRUE_)) {
    ...
    while (tau_upper - tau_lower > ppr->tol_tau_approx) {
      ...
      /* For PRTOE, use the same soft-start logic */
...
int prtoe_perturbations_derivs(
  ...
  if (pba->de_mode == prtoe_active) {
    ...
    double m_eff2 = V_phiphi
                  + (F_phiphi / F) * pba->R_curvature
                  - (F_phi*F_phi)/(F*F) * pba->R_curvature
                  - (beta_screened * phi * pba->R_curvature) / (pba->M_ew_prtoe * pba->M_ew_prtoe);
